    ...
    for (;;) {
        ...
        switch (escolha) {
            case 0:
                ...
                break;
            case 1:
                configSons(&cfg->sonsAtivos, *tmX, *tmY);
                break;
            case 2:
                clrRec(cfg->rec, *tmX, *tmY);
                break;
            case 3:
                textcolor(WHITE);
                return;
...
char confgVideo(char *corAleatoria, int *tmX, int *tmY) {
    ...
    for (;;) {
        ...
        switch (escolha) {
            case 0:
                confgVideoCorAleatoria(corAleatoria, *tmX, *tmY);
                break;
            case 1:
...
void ajustarTela(int *tmX, int *tmY) {
    char c, opcoes[4][30] = {"TAMANHO EM X",
                             "TAMANHO EM Y",
                             "TAMANHO DOS BLOCOS",
                             "VOLTAR"
    };
...
char pauseTela(Config *gamingConfig, int *tmX, int *tmY) {
    Musica mario;
    char mus = 0, escolha = 1, opcoes[3][30] = {{"CONFIGURACOES"},
                                                {"VOLTAR AO JOGO "},
                                                {"SAIR DO JOGO "},
    };
    ...
    printf("JOGO PAUSADO");
    for (;;) {
        escolha = fazerMenuMario(opcoes, 3, *tmX / 3 + 1, *tmY / 3 + 1, LIGHTGREEN, LIGHTCYAN, LIGHTMAGENTA, *tmX,
                                 *tmY);
        switch (escolha) {
            case 0 :
                menuConfig(gamingConfig, tmX, tmY);
                textbackground(FUNDO);
                system("cls");
                fazerQuadrado(1, 1, *tmX, *tmY);
                fazerQuadrado(*tmX / 3, *tmY / 3 - 4, *tmX / 4, 2);
    ...
    for (;;) {
        if (animacao) {
            ...
            if (tf - t0 > 100) {
                ...
                switch (pc) {
                    case 0:
                        printaIMG(&peca1, x0, y0);
                        break;
                    case 1:
                        printaIMG(&peca2, x0, y0);
                        break;
                    case 2:
                        printaIMG(&peca3, x0, y0);
                        break;
                }
                x0 -= 2;
